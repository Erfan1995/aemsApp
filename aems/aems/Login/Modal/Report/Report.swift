...
class Report {
    ...
    init(id:Int?,latitude:Double?,longitude:Double?,observer_id:Int?,void_vote:Int?,white_vote:Int?,right_vote:Int?,province_id:Int?,polling_center_id:Int?,pc_station_nummber:Int?,date_time:String?) {
            self.id=id
    ...
    static let CREATE_TABLE=" CREATE TABLE IF NOT EXISTS \(TABLE_NAME) ( \(COL_ID) INTEGER PRIMARY KEY AUTOINCREMENT ,\(COL_POLLING_CENTER) INTEGER , \(COL_STATION) INTEGER , \(COL_OBSERVER_ID) INTEGER , \(COL_VOID_VOTE) INTEGER , \(COL_WHITE_VOTE) INTEGER ,\(COL_RIGHT_VOTE) INTEGER , \(COL_DATE_TIME) TEXT , \(COL_LATITUDE) DOUBLE , \(COL_LONGITUDE) DOUBLE , \(COL_PROVINCE_ID) INTEGER , \(COL_IS_SENT) BOOLEAN DEFAULT FALSE ); "
